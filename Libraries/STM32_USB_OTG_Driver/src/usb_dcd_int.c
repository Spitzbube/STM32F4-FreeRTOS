...
#if 1
    if (!(gintr_status.b.sofintr/*0x08*/))
#endif
    {
    	printf("IRQ: 0x%x\n\r", gintr_status.d32);
    }
...
//  printf("DCD_HandleUSBSuspend_ISR\n\r");
...
static uint32_t DCD_HandleRxStatusQueueLevel_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  printf("DCD_HandleRxStatusQueueLevel_ISR\n\r");
  printf("GRXSTSP Status=%d, epnum=%d, pktsts=%d\n\r", status.d32, status.b.epnum, status.b.pktsts);
  ...
  case STS_DATA_UPDT:
...
#if 1
      for (int i = 0; i < status.b.bcnt; i++)
      {
      	printf("xfer_buff[%d]=0x%x\n\r", i, ep->xfer_buff[i]);
      }
#endif
    ...
    {
    	printf("STS_DATA_UPDT: 0\n\r");
    }
    break;
  case STS_XFER_COMP:
  	printf("STS_XFER_COMP\n\r");
    break;
  case STS_SETUP_COMP:
	printf("STS_SETUP_COMP\n\r");
    break;
  case STS_SETUP_UPDT:
    ...
    for (int i = 0; i < 8; i++)
    {
    	printf("setup_packet[%d]=0x%x\n\r", i, pdev->dev.setup_packet[i]);
    }
#endif
    ep->xfer_count += status.b.bcnt;
    break;
